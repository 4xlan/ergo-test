...
func factory_testSup() gen.ProcessBehavior {
	return &testSup{}
}
...
func (sup *testSup) Init(args ...any) (act.SupervisorSpec, error) {
	...
	spec.Children = []act.SupervisorChildSpec{
		{
			Name:    "act1",
			Factory: factory_Act1,
		},
		{
			Name:    "act2",
			Factory: factory_Act2,
		},
		{
			Name:    "act3",
			Factory: factory_Act3,
		},
	}
...
func (sup *testSup) HandleChildStart(name gen.Atom, pid gen.PID) error {
	return nil
}

// HandleChildTerminate invoked on a child process termination if option EnableHandleChild
// was enabled in act.SupervisorSpec
func (sup *testSup) HandleChildTerminate(name gen.Atom, pid gen.PID, reason error) error {
	return nil
}
...
func (sup *testSup) HandleMessage(from gen.PID, message any) error {
...
func (sup *testSup) HandleCall(from gen.PID, ref gen.Ref, request any) (any, error) {
...
func (sup *testSup) Terminate(reason error) {
	sup.Log().Info("supervisor terminated with reason: %s", reason)
}

// HandleInspect invoked on the request made with gen.Process.Inspect(...)
func (sup *testSup) HandleInspect(from gen.PID, item ...string) map[string]string {
